...
$sql = "SELECT * FROM carsharetrips ORDER BY trip_id DESC";
$result = mysqli_query($con, $sql);

if($result && mysqli_num_rows($result) > 0){

    echo '<h3 class="mt-5 mb-4">Available Rides</h3>';

    while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){

        $person_id = $row['user_id'];
        $sqlUser = "SELECT * FROM users WHERE user_id='$person_id' LIMIT 1";
            ...
            <div class='card-body'>
                <div class='row'>
                    <div class='col-md-2 text-center'>
                        <img src='$picture' style='height:60px;width:60px' class='rounded-circle'><br>
                        <small>$firstname</small>
                    </div>
                    <div class='col-md-6'>
                        <strong class='text-success'>Departure:</strong> {$row['departure']}<br>
                        <strong class='text-danger'>Destination:</strong> {$row['destination']}<br>
                        <span class='text-primary'>{$row['date']} at {$row['time']}</span>
                    </div>
                    <div class='col-md-4 text-end'>
                        <h5 class='text-danger'>₹ {$row['price']}</h5>
                        <small>{$row['seatsavailable']} seats left</small>
                    </div>
                </div>
            </div>
        </div>";
    ...
    echo "<p class='text-muted mt-4'>No rides available yet.</p>";
}
?>
