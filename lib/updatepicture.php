...
function uploadPrifilePicture($id,$file,$ext,$con)
{
    ...
    // 1️ Server path (for moving file)
    $serverPath = "../profilepicture/" . $filename;

    // 2️ Browser path (for saving in DB)
    $dbPath = "profilepicture/" . $filename;

    if(move_uploaded_file($file,$serverPath))
    {
        $sql="UPDATE users SET profilepicture='$dbPath' WHERE user_id='$id'";
        $result=mysqli_query($con,$sql);
        if(!$result)
        {
            echo '<div class="alert alert-danger">Unable to update database. Please try again!</div>';  
        }
    }
    else
    {
        echo '<div class="alert alert-warning">Unable to upload file. Please try again!</div>';  
    }
}
